...
export default function Courses() {
  ...
  const loadData = async (resolvedStudentId = null, silent = false) => {
    ...
    try {
      ...
      console.warn("Backend API failed to load courses/enrollments, trying direct Supabase fallback:", err);
      try {
        const { data: coursesResData } = await supabase.from("courses").select("*");
        coursesData = coursesResData || [];
        ...
        const { data: directEnrollmentsData } = await supabase.from("enrollments").select("*").eq("student_id", activeStudentId);
        enrollmentsData = directEnrollmentsData || [];
      } catch (dbErr) {
    ...
    // --- ENROLLMENTS & COMPLETED LESSONS SYNCHRONIZATION ---
    try {
      // 1. Sync guest completed lessons if any
      ...
      if (combinedGuestCompleted.length > 0) {
        const userCompleted = JSON.parse(localStorage.getItem(`edumind_completed_lessons_${activeStudentId}`) || "[]");
        const combinedCompleted = Array.from(new Set([...userCompleted, ...combinedGuestCompleted]));
        localStorage.setItem(`edumind_completed_lessons_${activeStudentId}`, JSON.stringify(combinedCompleted));
        localStorage.removeItem("edumind_completed_lessons_undefined");
        localStorage.removeItem("edumind_completed_lessons_guest");
        setCompletedLessons(combinedCompleted);
      } else {
      ...
      // 2. Sync guest and local enrollments to the database
      ...
      if (combinedLocal.length > 0) {
        ...
        for (const courseId of combinedLocal) {
          if (!enrollmentsData.some(e => e.course_id === courseId || e.courses?.id === courseId)) {
            try {
              ...
              try {
                ...
                console.warn(`Failed to sync course ${courseId}:`, dbErr);
              }
            }
          }
        }
        if (hasNewSync) {
          try {
            const enrollRes = await api.get(`/courses/my-courses/${activeStudentId}`);
            enrollmentsData = enrollRes.data || enrollmentsData;
          } catch (err) {
            const enrollmentsRes = await supabase.from("enrollments").select("*, courses(*)").eq("student_id", activeStudentId);
            enrollmentsData = enrollmentsRes.data || enrollmentsData;
          }
        }
        
        // Save database list to local cache
        const allDbCourseIds = enrollmentsData.map(e => e.course_id);
        localStorage.setItem(`edumind_local_enrollments_${activeStudentId}`, JSON.stringify(allDbCourseIds));
      ...
      console.warn("Enrollment sync failed:", syncErr);
    }
  ...
  const handleEnroll = async (courseId) => {
    const activeStudentId = studentId || JSON.parse(localStorage.getItem("edumind_user") || "{}").id;
    if (!activeStudentId) {
      // Guest enrollment: save to guest list and show toast
      const guestEnrollments = JSON.parse(localStorage.getItem("edumind_local_enrollments_guest") || "[]");
      if (!guestEnrollments.includes(courseId)) {
        localStorage.setItem("edumind_local_enrollments_guest", JSON.stringify([...guestEnrollments, courseId]));
      }
      toast.success("Enrolled locally! Sign in to sync your progress to your account. 🚀");
  ...
  const isEnrolled = (courseId) => {
    ...
    const guestEnrollments = JSON.parse(localStorage.getItem("edumind_local_enrollments_guest") || "[]");
    return enrollments.some(e => e.course_id === courseId || e.courses?.id === courseId) || localEnrollments.includes(courseId) || guestEnrollments.includes(courseId);
  };
...
*Note: Professor ARIA has configured fallback modules. Complete course sheets are active!*`
  ...
  const endQuiz = async () => {
    ...
    const saveQuizResultLocally = () => {
      const key = `edumind_local_quiz_results_${targetStudentId}`;
      const localResults = JSON.parse(localStorage.getItem(key) || "[]");
      localResults.push(localQuizResult);
      localStorage.setItem(key, JSON.stringify(localResults));
      window.dispatchEvent(new Event("edumind_db_sync"));
    };

    let submitPromise;
    if (targetStudentId === "guest") {
      saveQuizResultLocally();
      submitPromise = Promise.resolve();
    } else {
      submitPromise = api.post(`/students/quiz-result?student_id=${targetStudentId}`, {
        course_id: activeCourseId || null,
        subject: quizSubject,
        topic: quizAnswers[0]?.topic || "General Assessment",
        score: finalScore,
        ...
        console.error("Failed to submit quiz results online, saving locally:", err);
        saveQuizResultLocally();
      });
    }
                          ...
                          <div className="space-y-4 mt-4">
                            {riskResult.recommendations.map((rec, rIdx) => (
                              <div key={rIdx} className="flex gap-3 items-start text-xs text-gray-300">
                                ...
                                <p className="leading-relaxed">{cleanMathLaTeX(rec)}</p>
                              </div>
                            ))}
                          </div>
                        </div>
